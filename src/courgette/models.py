...
# --------------------------------------------------------------------------- #
# SyncMeteo — suivi de la dernière synchronisation météo (une seule ligne)
# --------------------------------------------------------------------------- #

class SyncMeteo(SQLModel, table=True):
    """Une seule ligne (id=1) : jusqu'à quel jour on a déjà traité la pluie réelle."""

    id: int | None = Field(default=None, primary_key=True)
    derniere_synchro: date = Field(description="Dernier jour (passé) déjà traité pour créer les arrosages automatiques.")